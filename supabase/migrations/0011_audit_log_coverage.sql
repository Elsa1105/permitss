...
-- REPLACE your current 0011_audit_log_coverage.sql with this file.
-- Fix included:
-- - audit_user_updates uses audit_action enum, not text.
-- - Adds photo update audit for annotation persistence.
-- - Keeps audit coverage for documents, permit header updates, user updates, and site roles.

-- =====================================================
-- 1. Audit permit photo insert/update/delete
-- =====================================================

-- Add enum value safely if your PostgreSQL version supports it.
-- This is not used by older functions until after commit.
do $$
begin
...
begin
  ...
  if tg_op = 'UPDATE' then
    -- Use existing permit_updated action to avoid enum value timing issues.
    perform public.write_audit(
      new.permit_id,
      'permit_updated',
      null,
      ...
      null,
      jsonb_build_object(
        'change_type', 'photo_updated',
        'photo_id', new.id,
        'storage_path', new.storage_path,
        'annotation_changed', old.annotation_data is distinct from new.annotation_data
      )
    );
    return new;
  end if;
...
begin
  -- Avoid duplicating normal state transition audit.
  if old.state is distinct from new.state
     and old.vessel_project is not distinct from new.vessel_project
     and old.location_of_work is not distinct from new.location_of_work
     and old.date_commencement is not distinct from new.date_commencement
     and old.date_completion is not distinct from new.date_completion
     and old.description is not distinct from new.description
     and old.hazard_types is not distinct from new.hazard_types
     and old.contractor is not distinct from new.contractor
  then
    return new;
  end if;

  if old.vessel_project is distinct from new.vessel_project then
    v_changed := v_changed || jsonb_build_object('vessel_project', jsonb_build_object('old', old.vessel_project, 'new', new.vessel_project));
  end if;

  if old.location_of_work is distinct from new.location_of_work then
    v_changed := v_changed || jsonb_build_object('location_of_work', jsonb_build_object('old', old.location_of_work, 'new', new.location_of_work));
  end if;

  if old.date_commencement is distinct from new.date_commencement then
    v_changed := v_changed || jsonb_build_object('date_commencement', jsonb_build_object('old', old.date_commencement, 'new', new.date_commencement));
  end if;

  if old.date_completion is distinct from new.date_completion then
    v_changed := v_changed || jsonb_build_object('date_completion', jsonb_build_object('old', old.date_completion, 'new', new.date_completion));
  end if;

  if old.description is distinct from new.description then
    v_changed := v_changed || jsonb_build_object('description', jsonb_build_object('old', old.description, 'new', new.description));
  end if;

  if old.hazard_types is distinct from new.hazard_types then
    v_changed := v_changed || jsonb_build_object('hazard_types', jsonb_build_object('old', old.hazard_types, 'new', new.hazard_types));
  end if;

  if old.contractor is distinct from new.contractor then
    v_changed := v_changed || jsonb_build_object('contractor', jsonb_build_object('old', old.contractor, 'new', new.contractor));
  end if;

  if old.company_id is distinct from new.company_id then
    v_changed := v_changed || jsonb_build_object('company_id', jsonb_build_object('old', old.company_id, 'new', new.company_id));
  end if;

  if old.site_id is distinct from new.site_id then
    v_changed := v_changed || jsonb_build_object('site_id', jsonb_build_object('old', old.site_id, 'new', new.site_id));
  end if;

  if old.contractor_company is distinct from new.contractor_company then
    v_changed := v_changed || jsonb_build_object('contractor_company', jsonb_build_object('old', old.contractor_company, 'new', new.contractor_company));
  end if;

  if old.contractor_supervisor_name is distinct from new.contractor_supervisor_name then
    v_changed := v_changed || jsonb_build_object('contractor_supervisor_name', jsonb_build_object('old', old.contractor_supervisor_name, 'new', new.contractor_supervisor_name));
  end if;

  if old.contractor_supervisor_registration_no is distinct from new.contractor_supervisor_registration_no then
    v_changed := v_changed || jsonb_build_object('contractor_supervisor_registration_no', jsonb_build_object('old', old.contractor_supervisor_registration_no, 'new', new.contractor_supervisor_registration_no));
  end if;

  if old.worker_briefing_acknowledged is distinct from new.worker_briefing_acknowledged then
    v_changed := v_changed || jsonb_build_object('worker_briefing_acknowledged', jsonb_build_object('old', old.worker_briefing_acknowledged, 'new', new.worker_briefing_acknowledged));
  end if;

  if old.top_controls_summary is distinct from new.top_controls_summary then
    v_changed := v_changed || jsonb_build_object('top_controls_summary', jsonb_build_object('old', old.top_controls_summary, 'new', new.top_controls_summary));
  end if;
  ...
  if v_changed <> '{}'::jsonb then
    perform public.write_audit(
      ...
      new.state,
      null,
      jsonb_build_object('changed_fields', v_changed, 'updated_at', now())
    );
  end if;
...
begin
  if old.active is distinct from new.active then
    ...
    v_changed := v_changed || jsonb_build_object('active', jsonb_build_object('old', old.active, 'new', new.active));
  end if;

  if old.role is distinct from new.role then
    v_changed := v_changed || jsonb_build_object('role', jsonb_build_object('old', old.role, 'new', new.role));
  end if;

  if old.full_name is distinct from new.full_name then
    v_changed := v_changed || jsonb_build_object('full_name', jsonb_build_object('old', old.full_name, 'new', new.full_name));
  end if;

  if old.department is distinct from new.department then
    v_changed := v_changed || jsonb_build_object('department', jsonb_build_object('old', old.department, 'new', new.department));
  end if;

  if old.qualified_for is distinct from new.qualified_for then
    v_changed := v_changed || jsonb_build_object('qualified_for', jsonb_build_object('old', old.qualified_for, 'new', new.qualified_for));
  end if;
...
drop trigger if exists trg_audit_user_site_roles_insert on public.user_site_roles;
...
drop trigger if exists trg_audit_user_site_roles_update on public.user_site_roles;
create trigger trg_audit_user_site_roles_update
after update on public.user_site_roles
for each row
execute function public.audit_user_site_roles();

drop trigger if exists trg_audit_user_site_roles_delete on public.user_site_roles;
