-- =====================================================
-- Migration 0012: Client Revisions Core (FIXED FINAL)
-- =====================================================
...
-- =====================================================
-- 2. ENUM SAFE (document type)
-- =====================================================

do $$
begin
  if not exists (
    select 1 from pg_type where typname = 'permit_document_type'
  ) then
...
-- =====================================================
-- 5. RLS POLICIES (SAFE + CONSISTENT)
-- =====================================================
...
for each row execute function public.audit_permit_document_change();
...
-- =====================================================
-- 7. AUDIT FUNCTION: PHOTO (SAFE UPDATE CHECK)
-- =====================================================
...
for each row execute function public.audit_permit_photo_change();
...
declare
  ...
  v_from_state permit_state;
  v_to_state permit_state;
...
begin
  ...
  if v_from_state <> 'pending_safety_assessment' then
    raise exception 'Invalid state: %', v_from_state;
  end if;
